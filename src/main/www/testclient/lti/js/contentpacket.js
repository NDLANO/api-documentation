...
function updateSlideshowProgressBar(slideshowContainer){
...
function drawFrontpageProgressBar(slideshowContainer){
...
function drawSlideshowProgressBar(slideshowContainer){
...
function updateSlideshowState(slideshowContainer){
    var numSlides = slideshowContainer.find(".slideshow .slide").size();
...
function indexSlides(slideshowContainer){
...
function nextSlide(slideshowContainer){
    ...
    updateSlideshowState(slideshowContainer);
}
...
function setStateToFrontpage(slideshowContainer){
    slideshowContainer.find(".slideshow").hide();
    slideshowContainer.find(".slideshow-frontpage").show();
}

function setStateToSlideshow(slideshowContainer){
    location.hash = "1";
    slideshowContainer.find(".slideshow .slide")
        .hide()
        .first()
    ...
    updateSlideshowProgressBar(slideshowContainer);
}
...
function startSlideshow(slideshowContainer){
    ...
    slideshowContainer.find(".slideshow .slideshow-progressbar svg .progressbar-slide")
        ...
        .first()
        .attr("fill", "url(#activeSlideGradient)");
    setStateToSlideshow(slideshowContainer);
}

function prepareSlideshow(slideshowContainer){
    $(window).on("hashchange", function(){
        if(location.hash == "#1"){
            setStateToSlideshow(slideshowContainer);
        }
        else {
            setStateToFrontpage(slideshowContainer);
        }
    });
    $(window).resize(function(){updateSlideshowProgressBar(slideshowContainer);});
    indexSlides(slideshowContainer);
    drawFrontpageProgressBar(slideshowContainer);
    drawSlideshowProgressBar(slideshowContainer);
    setStateToFrontpage(slideshowContainer);
}
